...
const FundingForm = ({ data, onNext, onBack }) => {
  ...
  const formik = useFormik({
    initialValues: {
      ...
      st_raised_to_date: data.st_raised_to_date.split(" ")[0] || "", // Extract amount only
      st_currency: data.st_currency || "USD",
      st_last_valuation: data.st_last_valuation.split(" ")[0] || "", // Extract amount only
      st_current_valuation: data.st_current_valuation.split(" ")[0] || "", // Extract amount only
      st_capital_requirements: data.st_capital_requirements.split(" ")[0] || "", // Extract amount only
      amountUnit: data.st_raised_to_date.split(" ")[1] || "Th",  // Extract unit
      lvamountUnit: data.st_last_valuation.split(" ")[1] || "Th",  // Extract unit
      cvamountUnit: data.st_current_valuation.split(" ")[1] || "Th",  // Extract unit
      cramountUnit: data.st_capital_requirements.split(" ")[1] || "Th",  // Extract unit
    },
    validationSchema,
  ...
  return (
    <Card>
      <CardBody>
        ...
        <Form className="fundingForm" onSubmit={formik.handleSubmit} noValidate>
